...
class Lot(Base):
    ...
    product_catalog_id = Column(Integer, ForeignKey("product_catalog.id"))
    wood_essence_id = Column(Integer, ForeignKey("essence_catalog.id"))
    wood_form = Column(String(40))
    volume_m3 = Column(Numeric(14, 4))
    ...
    lot_number = Column(String(120), unique=True)
    traceability_id = Column(String(120), unique=True)
    origin_reference = Column(String(160))
    previous_block_hash = Column(String(64))
    current_block_hash = Column(String(64))
    trace_payload_json = Column(Text)
    wood_classification = Column(String(30))
    cites_laf_status = Column(String(20))
    cites_ndf_status = Column(String(20))
    cites_international_status = Column(String(20))
    destruction_status = Column(String(20))
    destruction_requested_at = Column(DateTime(timezone=True))
    destruction_validated_at = Column(DateTime(timezone=True))
    destruction_evidence_json = Column(Text)
