...
export default function ActorsPage() {
  ...
  return (
    <div>
      <h1 style={{ marginBottom: '30px' }}>Acteurs</h1>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {data?.items?.map((actor: any) => (
              <tr key={actor.id} style={{ borderBottom: '1px solid #eee' }}>
                <td style={{ padding: '10px' }}>{actor.id}</td>
                <td style={{ padding: '10px' }}>
                  {actor.nom} {actor.prenoms}
                </td>
                <td style={{ padding: '10px' }}>{actor.email}</td>
                <td style={{ padding: '10px' }}>{actor.telephone}</td>
                <td style={{ padding: '10px' }}>{actor.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
